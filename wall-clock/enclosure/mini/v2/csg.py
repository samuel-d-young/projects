...
# ---------------------------------------------------------------- where files live
# Sam, 2026-09-05: "Group the STL and 3Mf files into their own folders." 47 STLs
# and 45 3MFs in one directory alongside the source made the folder unusable.
#
#   stl/     generated meshes, safe to delete and rebuild
#   3mf/     the same parts with their print orientation, ditto
#   input/   THE TWO MESHES SAM UPLOADED. Not generated, not reproducible from
#            this repo, and the reason they are not in stl/: that folder is
#            disposable and these are the only files here that cannot be
#            regenerated by running build_v2.py.
_HERE   = os.path.dirname(os.path.abspath(__file__))
_INPUTS = {'base_in.stl', 'diffuser_in.stl'}
_DIRS   = {'.stl': 'stl', '.3mf': '3mf'}

def part(name):
    """Resolve a bare part filename to its folder. Idempotent: anything that
    already carries a directory is handed back untouched, so call sites that
    are given a real path still work."""
    if os.path.dirname(name):
        return name
    if name in _INPUTS:
        return os.path.join(_HERE, 'input', name)
    return os.path.join(_HERE, _DIRS.get(os.path.splitext(name)[1].lower(), ''), name)

def part_out(name):
    """Same, but makes the directory first -- for writing."""
    p = part(name)
