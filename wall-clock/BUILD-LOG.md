...
## 2026-09-17 — "Do this for me", and the generated file I had been hand-editing

Sam asked to flash all three for the third time, then: *"Do thid for me"*.

I cannot, and this time I tested all three routes rather than repeating the
sentence: no LAN route (this container is `192.0.2.2`; `.75`, `.69` and `.68`
all refuse 6053), **one environment and it is `anthropic_cloud`**, and
`ListAgents` finds no session of Sam's running on a machine at home. Every door
is shut, and no amount of willingness opens one.

So the useful answer is not another set of instructions. It is to make the
instructions unnecessary.

### One button

`packages/wall_clock_update.yaml` — `script.wall_clock_update_all`, and a
**Firmware (all clocks)** card on the Settings view.

It **discovers** the update entities rather than naming them:

```jinja
{{ integration_entities('esphome') | select('match', 'update\.')
   | select('search', 'mini_round_clock') | list }}
```

The obvious version hard-codes `update.mini_round_clock_3_firmware`, and I
cannot verify that id from here — which is precisely the kind of guess that has
already cost this project a sheet of plywood. `integration_entities` resolves on
the box. If it finds nothing it raises a notification **saying so**, instead of
being a button that does nothing. One clock at a time, five seconds apart:
three simultaneous OTA builds is how you discover how much RAM the add-on
wanted.

Still unverified, and it will tell you which: the ESPHome `update` entities
only exist if the Device Builder add-on is installed and the integration has a
dashboard configured.

### The thing I found on the way, which is worse

`wall-clock-settings-view.yaml` opens with **"Generated by
build_clock_dashboard.py — do not hand-edit"**.

I hand-edited it. Three times today: the per-face weather switches, the stars
direction, the stars-count-from. None of them existed in the generator. **The
next person to run that script would have silently deleted every control Sam
asked for today**, and the only symptom would be rows quietly missing from a
dashboard.

All three are in the generator now, and re-running it reproduces them. The
label indent is U+2003 EM SPACE and not three dots — same trap as the first time
I edited this file, and I fell in it again by typing `...` where the house style
is an em space.

And the regeneration found a second one. The sections layout collected its
global cards like this:
...
A filter that names **one member of a category** is a filter that is wrong the
moment the category gains a second. My firmware card was written, generated, and
simply not in the output — no error, no warning, nothing to notice. It selects
every global entities card now.

**The lesson, and it is the day's third variation on it:** a file that says it
