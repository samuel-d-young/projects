...
2. **The display module's overall thickness.** `DISP_T` = 5.60 in `v2/params.py` is what the diffuser's collar length is derived from — the collar tip sits **0.90 mm** clear of `Z_SEAT + DISP_T`, which puts it at 15.10 against a module front face at 14.20. Sam has reported the collar too long **four times**, which is what drove it from 8.20 to 3.93; if his module is thicker than 5.60, change `DISP_T` and rebuild. Do not change `COLLAR_LEN` or `COLLAR_RIB_Z1` — both are derived from it.
...
7. **The round panel's colour order.** `color_order: rgb` on `face_lcd` is the fix for Sam's *"the colours seem to be opposite on the screen"*, and it is reasoned rather than measured: xboot's reference driver sets MADCTL `0x00` (RGB), ESPHome appends its own MADCTL over the init table's, and this panel's own repo records the no-`color_order` default resolving to BGR. If the hands come out blue-on-orange the other way, flip this one word back. **`Face → colour test`** puts three labelled swatches on the panel — green in the middle, because green is the channel an R/B swap never touches — so one look tells you whether it is colour order or `invert_colors`.
